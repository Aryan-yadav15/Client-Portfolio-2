...
export const InfiniteMovingCards = ({
  ...
  const getSpeed = () => {
    ...
    if (scrollerRef.current) {
      scrollerRef.current.style.setProperty("--animation-duration", `${scrollDuration}s`);
    }
  };
  ...
  return (
    ...
    >
      <ul
        ref={scrollerRef}
        className={`flex min-w-full shrink-0 gap-14 xlg:gap-20 py-4 w-max ${start ? "scroll-animation" : ""} ${pauseOnHover ? "hover:animation-play-state-paused" : ""}`}
      >
        {items.map((item, idx) => (
          <li key={idx}>
            <Image
              ...
              src={item.href}
              alt={item.href}
              className="relative rounded-2xl object-contain opacity-70 w-[50px] h-[50px]"
            />
          </li>
        ))}
      </ul>
      <style jsx>{`
        @keyframes scroll {
          ...
          to {
            transform: translateX(-50%); /* Change to -50% to show the entire first set */
          }
        }
